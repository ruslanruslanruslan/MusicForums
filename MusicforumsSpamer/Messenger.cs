...
namespace MusicforumsSpamer
{
    class Messenger
    {
        ...
        public void ClearRepeats()
        {
            ...
            {
                string to = Regex.Match(item, "&to=(.+)").Groups[1].Value;
        ...
        {
            string to = Regex.Match(link, "&to=(.+)").Groups[1].Value;
            string bn = Regex.Match(link, "\\?bn=(.+?)&").Groups[1].Value;
            string postString = string.Format("do_post=1&actiontype=insert&bn={0}&to={1}&email=&subject={2}&icon=icon1.gif&img_url=&body={3}&save=1", bn, to, message.Title, message.Body);
            HttpWebRequest req = (HttpWebRequest)HttpWebRequest.Create(urlSend);
         ...
         //   req.Referer = link;
            ...
            bool isGood = Regex.IsMatch(content, "\\{\"error\":0");
            if (!isGood) 
            {
                using (TextWriter sw = new StreamWriter("errorLinks.txt",true))
                {
                    sw.WriteLine(link);
                }
            }
            ...
            {
                using (TextWriter sw = new StreamWriter("users.txt", true))
                {
                    sw.WriteLine(to);
                }
            }
